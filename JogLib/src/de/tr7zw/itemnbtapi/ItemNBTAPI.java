...
/*
 * Changes made for use in JogLib
 * 
 * Changed this class to no longer implement JavaPlugin
 * onEnable method renamed to init, and called by the JogLib onEnable method
 * init method accepts instance of the JogLib Plugin object for use in calling methods from the Plugin object (such as getLogger();)
 * jogLibPlugin value added to store the plugin instance received in the init method
 * MetricsLite removed
 */

public class ItemNBTAPI
{
	private static Plugin jogLibPlugin;
	...
	public void init(Plugin plugin)
	{
		...
		instance = this;
		jogLibPlugin.getLogger().info("Checking bindings...");
		jogLibPlugin.getLogger().info("Minecraft Version:");
		MinecraftVersion.getVersion();
		jogLibPlugin.getLogger().info("Gson:");
		MinecraftVersion.hasGsonSupport();
		jogLibPlugin.getLogger().info("Classes:");
		for (ClassWrapper c : ClassWrapper.values())
		{
			if (c.getClazz() == null)
			{
				jogLibPlugin.getLogger().warning(c.name() + " did not find it's class!");
				compatible = false;
			}
		}
		jogLibPlugin.getLogger().info("Methods:");
		for (ReflectionMethod method : ReflectionMethod.values())
		{
			if (method.isCompatible() && !method.isLoaded())
			{
				jogLibPlugin.getLogger().warning(method.name() + " did not find the method!");
				compatible = false;
			}
		}
		jogLibPlugin.getLogger().info("Running NBT reflection test...");
		try
		{
			// Item
			...
			{
				jogLibPlugin.getLogger().warning(
			...
			{
				jogLibPlugin.getLogger().warning("Wasn't able to check a key! The Item-NBT-API may not work!");
				compatible = false;
			}
			...
			{
				jogLibPlugin.getLogger().warning("One key does not equal the original value! The Item-NBT-API may not work!");
				compatible = false;
			}
			nbtItem.setString(STRING_TEST_KEY, null);
			if (nbtItem.getKeys().size() != 10)
			{
				jogLibPlugin.getLogger().warning("Wasn't able to remove a key (Got " + nbtItem.getKeys().size()
			...
			if (comp == null)
			{
				jogLibPlugin.getLogger().warning("Wasn't able to get the NBTCompound! The Item-NBT-API may not work!");
				compatible = false;
			}
			if (!comp.hasKey(STRING_TEST_KEY))
			{
				jogLibPlugin.getLogger().warning("Wasn't able to check a compound key! The Item-NBT-API may not work!");
				compatible = false;
			}
			...
			{
				jogLibPlugin.getLogger().warning("Wasn't able to get the correct Tag type! The Item-NBT-API may not work!");
				compatible = false;
			}
			if (!list.getString(1).equals("test42") || list.size() != 3)
			{
				jogLibPlugin.getLogger().warning("The List support got an error, and may not work!");
			}
			...
			if (taglist.size() == 1)
			{
				...
				{
					jogLibPlugin.getLogger().warning("Wrong key amount in Taglist (" + lcomp.getKeys().size()
				...
				{
					jogLibPlugin.getLogger().warning("One key in the Taglist changed! The Item-NBT-API may not work!");
					compatible = false;
				}
			}
			else
			{
				jogLibPlugin.getLogger().warning("Taglist is empty! The Item-NBT-API may not work!");
				compatible = false;
			}
		}
		catch (Exception ex)
		{
			jogLibPlugin.getLogger().log(Level.SEVERE, null, ex);
			compatible = false;
		}
		...
		{
			// File
			NBTFile file = new NBTFile(new File(jogLibPlugin.getDataFolder(), "test.nbt"));
			...
			file.save();
			file = null;
			file = new NBTFile(new File(jogLibPlugin.getDataFolder(), "test.nbt"));
			...
			{
				jogLibPlugin.getLogger().warning("Wasn't able to parse NBT from a String! The Item-NBT-API may not work!");
				compatible = false;
			}
			...
			// ItemStack afteritem = NBTItem.convertNBTtoItem(new
			// NBTContainer(itemasString));
			// if(!preitem.isSimilar(afteritem)){
			// getLogger().warning("Wasn't able to convert an Item to String and back to
			// Item! The Item-NBT-API may not work!");
			// compatible = false;
			// }
			// mergingtags
			...
			{
				jogLibPlugin.getLogger().warning("Wasn't able to merge Compounds! The Item-NBT-API may not work!");
				compatible = false;
			}
			...
			if (!Bukkit.getWorlds().isEmpty())
			{
				...
				try
				{
					...
					{
						jogLibPlugin.getLogger().info("Testing Entity NBT!");
						NBTEntity nbte = new NBTEntity(
								world.getEntitiesByClasses(Animals.class, Monster.class).iterator().next());
						jogLibPlugin.getLogger().info(nbte.asNBTString());
						nbte.setString("INVALIDEKEY", "test");
						jogLibPlugin.getLogger().info("Entity NBT seems to work!");
					}
				}
				catch (Exception ex)
				{
					jogLibPlugin.getLogger().warning("Wasn't able to use NBTEntities! The Item-NBT-API may not work!");
					compatible = false;
					ex.printStackTrace();
				}
				/*
				 * try { Block block = world.getBlockAt(world.getSpawnLocation().getBlockX(),
				 * 255, world.getSpawnLocation().getBlockZ()); if (block.getType() ==
				 * Material.AIR) { getLogger().info("Testing Tile NBT!");
				 * block.setType(Material.CHEST); NBTTileEntity tile = new
				 * NBTTileEntity(block.getState()); getLogger().info(tile.asNBTString());
				 * tile.setString("Lock", "test"); getLogger().info(tile.asNBTString());
				 * block.setType(Material.AIR); getLogger().info("Tile NBT seems to work!"); } }
				 * catch (Exception ex) { getLogger().
				 * warning("Wasn't able to use NBTTiles! The Item-NBT-API may not work!");
				 * compatible = false; ex.printStackTrace(); }
				 */
			}
		...
		if (compatible)
		{
			if (jsonCompatible)
			{
				jogLibPlugin.getLogger().info("Success! This version of Item-NBT-API is compatible with your server.");
			}
			else
			{
				jogLibPlugin.getLogger().info(
		...
		{
			jogLibPlugin.getLogger().warning("WARNING! This version of Item-NBT-API seems to be broken with your Spigot version! "
					+ checkMessage);
		}
	...
	public void testJson()
	{
		...
		{
			jogLibPlugin.getLogger().log(Level.SEVERE, null, ex);
			jogLibPlugin.getLogger().warning(ex.getMessage());
			jsonCompatible = false;
		}
	}
